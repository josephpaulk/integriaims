...
if (isset($_GET["create2"])){ //
	...
	if (! $result)
		echo "<h3 class='error'>".lang_string ("Building cannot be created")."</h3>";
	else {
		echo "<h3 class='suc'>".lang_string ("Building has been created successfully")."</h3>";
...
if (isset($_GET["update2"])){ // if modified any parameter
	...
	if (! $result)
		echo "<h3 class='error'>".lang_string ("Building cannot be updated")."</h3>";
	else {
		echo "<h3 class='suc'>".lang_string ("Building updated ok")."</h3>";
...
if ((isset($_GET["create"]) OR (isset($_GET["update"])))) {
	...
	echo "<h2>".lang_string ("Building management")."</h2>";
	if ($id == -1){
		echo "<h3>".lang_string ("Create a new building")."</a></h3>";
		echo "<form method='post' action='index.php?sec=inventory&sec2=operation/inventory/building_detail&create2=1'>";
	}
	else {
		echo "<h3>".lang_string ("Update existing building")."</a></h3>";
		echo "<form method='post' action='index.php?sec=inventory&sec2=operation/inventory/building_detail&update2=1'>";
	...
	echo "<tr>";
	echo "<td class=datos>";
	echo lang_string ("Building name");
    ...
    if ((!isset($_GET["update"])) AND (!isset($_GET["create"]))){
	    ...
	    if (($result=mysql_query($sql1)) AND (mysql_num_rows($result) >0)){
	        ...
	        while ($row=mysql_fetch_array($result)){
                // Name
                $table->data[$counter][0] = "<b><a href='index.php?sec=inventory&sec2=operation/inventory/building_detail&update=".$row["id"]."'>".$row["name"]."</a></b>";
			...
			echo "<form method=post action='index.php?sec=inventory&sec2=operation/inventory/building_detail&create=1'>";
			echo "<input type='submit' class='sub next' name='crt' value='".lang_string("Create new building")."'>";
			echo "</form></td></tr></table>";
