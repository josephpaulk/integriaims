...
if ($user_id == "") {
	...
	echo '<table width="90%" class="listing"><tr>';
	...
	if (sizeof($incidencias) == 0){
		...
		if ($incidencias) {
			foreach ($incidencias as $incident) {
				...
				echo "<td>".$status[$incident["istatus"]]."</td>";
				echo "<td class=f9>".substr($incident["date_start"],0,11). " <br>" .substr($incident["date_end"],0,11)."</td>";
				echo "<td>".$incident["suma"]."</td>";
				$incident_user  += $incident["suma"];
				$this_incident = get_incident_workunit_hours($incident["iid"]);
				echo "<td>". $this_incident."</td>";
