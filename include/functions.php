	...
	if ($count > $block_size){
		...
		if ($index_page > 0){
			...
			if ($index_page_prev < 0)
				$index_page_prev = 0;
			echo '<a id="page_'.$index_page_prev.'" href="'.$url.'&offset='.$index_page_prev.'"><img src="'.$config["base_url"].'/images/control_rewind_blue.png" border=0 valign="bottom"></a>';
		}
		echo "&nbsp;";echo "&nbsp;";
		// Draw blocks markers
		...
		for ($i = $inicio_pag; $i < $index_limit; $i++) {
			...
			echo '<a id="page_'.$inicio_bloque.'" href="'.$url.'&offset='.$inicio_bloque.'">';
		...
		if (($paginacion_maxima == 1) AND (($index_counter - $i) > 0)) {
				...
				echo '<a id="page_'.$prox_bloque.'" href="'.$url.'&offset='.$prox_bloque.'">';
		...
		if (($count - $block_size) > 0){
			...
			echo '<a id="page_'.$myoffset.'" href="'.$url.'&offset='.$myoffset.'">';
...
function print_priority_flag_image ($priority, $return = false, $class = '', $base_dir = '') {
	$output = '';
	
	$output .= '<img class="priority-color '.$class.'" height="15" width="15" ';
	switch ($priority) {
	case 0:
		// Informative
		$output .= 'src="'.$base_dir.'images/pixel_gray.png" title="'.__('Informative').'" ';
		break;
	case 1:
		// Low
		$output .= 'src="'.$base_dir.'images/pixel_green.png" title="'.__('Low').'" ';
		break;
	case 2:
		// Medium
		$output .= 'src="'.$base_dir.'images/pixel_yellow.png" title="'.__('Medium').'" ';
		break;
	case 3:
		// Serious
		$output .= 'src="'.$base_dir.'images/pixel_orange.png" title="'.__('Serious').'" ';
		break;
	case 4:
		// Very serious
		$output .= 'src="'.$base_dir.'images/pixel_red.png" title="'.__('Very serious').'" ';
		break;
	case 10:
		// Maintance
		$output .= 'src="'.$base_dir.'images/pixel_blue.png" title="'.__('Maintance').'" ';
		break;
	default:
		// Default
		$output .= 'src="'.$base_dir.'images/pixel_gray.png" title="'.__('Unknown').'" ';
	}
