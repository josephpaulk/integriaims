...
function um_client_db_save_package ($package) {
	...
	$sql =& $db->prepare ('INSERT INTO '.DB_PREFIX.'tupdate_package ('.implode(',', array_keys ($fields)).') VALUES ('.implode(',', $replace).')');
...
function um_client_db_save_update ($update) {
	...
	$sql =& $db->prepare ('INSERT INTO '.DB_PREFIX.'tupdate ('.implode(',', $fields).') VALUES ('.implode(',', $replace).')');
