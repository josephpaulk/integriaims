...
function um_db_load_settings () {
	...
	$result =& $db->query ('SELECT * FROM '.DB_PREFIX.'tupdate_settings');
...
function um_db_update_setting ($key, $value = '') {
	...
	$sql =& $db->prepare ('SELECT COUNT(*) e FROM '.DB_PREFIX.'tupdate_settings WHERE `key` = ?');
	...
	if ($exists->e) {
		$sql =& $db->prepare ('UPDATE '.DB_PREFIX.'tupdate_settings SET value = ? WHERE `key` = ?');
		...
		$sql =& $db->prepare ('INSERT INTO '.DB_PREFIX.'tupdate_settings (value, `key`) VALUES (?, ?)');
		$result =& $db->execute ($sql, $values);
		if (PEAR::isError ($result)) {
			echo '<strong>Error</strong>: '.$result->getMessage ().'<br />';
			return false;
...
function um_db_get_latest_public_package ($id_package = '0') {
	...
	$sql =& $db->prepare ('SELECT * FROM '.DB_PREFIX.'tupdate_package WHERE status = ? AND id > ? ORDER BY id DESC LIMIT 1');
	$result =& $db->execute ($sql, $values);
	if (PEAR::isError ($result)) {
		echo '<strong>Error</strong>: '.$result->getMessage ().'<br />';
		return false;
...
function um_db_get_next_package ($id_package = '0', $development = false) {
	...
	$values = array ('public', $id_package);
	$sql =& $db->prepare ('SELECT * FROM '.DB_PREFIX.'tupdate_package WHERE status = ? AND id > ? ORDER BY id ASC LIMIT 1');
	$result =& $db->execute ($sql, $values);
	if (PEAR::isError ($result)) {
		echo '<strong>Error</strong>: '.$result->getMessage ().'<br />';
		return false;
...
function um_db_create_package ($description = '') {
	...
	$sql =& $db->prepare ('INSERT INTO '.DB_PREFIX.'tupdate_package (description) VALUES (?)');
...
function um_db_update_package ($id_package, $description = '', $status = 'disabled') {
	...
	$sql =& $db->prepare ('UPDATE '.DB_PREFIX.'tupdate_package SET description = ?, status = ? WHERE id = ?');
	$result =& $db->execute ($sql, $values);
	if (PEAR::isError ($result)) {
		echo '<strong>Error</strong>: '.$result->getMessage ().'<br />';
		return false;
...
function um_db_delete_package ($id_package) {
	...
	$sql =& $db->prepare ('DELETE FROM '.DB_PREFIX.'tupdate_package WHERE id = ?');
...
function um_db_get_package ($id_package) {
	...
	$sql =& $db->prepare ('SELECT * FROM '.DB_PREFIX.'tupdate_package WHERE id = ? LIMIT 1');
	$result =& $db->execute ($sql, $id_package);
	if (PEAR::isError ($result)) {
		echo '<strong>Error</strong>: '.$result->getMessage ().'<br />';
		return NULL;
...
function um_db_get_all_packages () {
	...
	$result =& $db->query ('SELECT * FROM '.DB_PREFIX.'tupdate_package');
	if (PEAR::isError ($result)) {
		echo '<strong>Error</strong>: '.$result->getMessage ().'<br />';
		return array ();
	}
...
function um_db_get_package_updates ($id_package) {
	...
	$sql =& $db->prepare ('SELECT * FROM '.DB_PREFIX.'tupdate WHERE id_update_package = ?');
	$result =& $db->execute ($sql, $id_package);
	if (PEAR::isError ($result)) {
		echo '<strong>Error</strong>: '.$result->getMessage ().'<br />';
		return NULL;
...
function um_db_create_package_log ($id_package, $client_key, $user_package, $result = 'query', $user_subscription = '', $description = '') {
	...
	$sql =& $db->prepare ('INSERT INTO '.DB_PREFIX.'tupdate_package_log (id_update_package, client_key, ip_address, user_package, user_subscription, result, description) VALUES (?, ?, ?, ?, ?, ?, ?)');
	$result =& $db->execute ($sql, $values);
	if (PEAR::isError ($result)) {
		return false;
	}
...
function um_db_get_all_package_logs () {
	...
	$result =& $db->query ('SELECT * FROM '.DB_PREFIX.'tupdate_package_log ORDER BY `timestamp` DESC');
	if (PEAR::isError ($result)) {
		echo '<strong>Error</strong>: '.$result->getMessage ().'<br />';
		return array ();
	}
...
function um_db_create_component ($type, $name, $path = '') {
	...
	$sql =& $db->prepare ('INSERT INTO '.DB_PREFIX.'tupdate_component (type, name, path) VALUES (?, ?, ?)');
	$result =& $db->execute ($sql, $values);
	if (PEAR::isError ($result)) {
		echo '<strong>Error</strong>: '.$result->getMessage ().'<br />';
		return false;
...
function um_db_update_component ($name, $path = '') {
	...
	$sql =& $db->prepare ('UPDATE '.DB_PREFIX.'tupdate_component SET path = ? WHERE name = ?');
	$result =& $db->execute ($sql, $values);
	if (PEAR::isError ($result)) {
		echo '<strong>Error</strong>: '.$result->getMessage ().'<br />';
		return false;
...
function um_db_delete_component ($name) {
	...
	$sql =& $db->prepare ('DELETE FROM '.DB_PREFIX.'tupdate_component WHERE name = ?');
...
function um_db_get_component ($name) {
	...
	$sql =& $db->prepare ('SELECT * FROM '.DB_PREFIX.'tupdate_component WHERE name = ? LIMIT 1');
	$result =& $db->execute ($sql, $name);
	if (PEAR::isError ($result)) {
		echo '<strong>Error</strong>: '.$result->getMessage ().'<br />';
		return NULL;
...
function um_db_get_all_components ($type = '') {
	...
	if ($type != '') {
		$sql =& $db->prepare ('SELECT * FROM '.DB_PREFIX.'tupdate_component WHERE type = ?');
		$result =& $db->execute ($sql, $type);
	} else {
		$result =& $db->query ('SELECT * FROM '.DB_PREFIX.'tupdate_component');
	}
	if (PEAR::isError ($result)) {
		echo '<strong>Error</strong>: '.$result->getMessage ().'<br />';
		return array ();
...
function um_db_create_component_db ($table_name, $field_name, $order, $component_name) {
	...
	$sql =& $db->prepare ('INSERT INTO '.DB_PREFIX.'tupdate_component_db (table_name, field_name, `order`, component) VALUES (?, ?, ?, ?)');
	$result =& $db->execute ($sql, $values);
	if (PEAR::isError ($result)) {
		echo '<strong>Error</strong>: '.$result->getMessage ().'<br />';
		return false;
...
function um_db_update_component_db ($id, $table_name = '', $field_name = '', $order = '') {
	...
	$sql =& $db->prepare ('UPDATE '.DB_PREFIX.'tupdate_component_db SET table_name = ?, field_name = ?, `order` = ? WHERE id = ?');
	$result =& $db->execute ($sql, $values);
	if (PEAR::isError ($result)) {
		echo '<strong>Error</strong>: '.$result->getMessage ().'<br />';
		return false;
...
function um_db_delete_component_db ($id) {
	...
	$sql =& $db->prepare ('DELETE FROM '.DB_PREFIX.'tupdate_component_db WHERE id = ?');
...
function um_db_get_component_db ($id_component_db) {
	...
	$sql =& $db->prepare ('SELECT * FROM '.DB_PREFIX.'tupdate_component_db WHERE id = ? LIMIT 1');
...
function um_db_get_database_components ($component_name) {
	...
	$sql =& $db->prepare ('SELECT * FROM '.DB_PREFIX.'tupdate_component_db WHERE component = ? ORDER BY `order` ASC');
...
function um_db_create_auth ($client_key, $subscription_limit, $description = '', $developer = false) {
	...
	$sql =& $db->prepare ('INSERT INTO '.DB_PREFIX.'tupdate_auth (client_key, subscription_limit, description, developer) VALUES (?, ?, ?, ?)');
	$result =& $db->execute ($sql, $values);
	if (PEAR::isError ($result)) {
		echo '<strong>Error</strong>: '.$result->getMessage ().'<br />';
		return false;
...
function um_db_update_auth ($id_auth, $client_key, $subscription_limit, $description = '', $developer = false) {
	...
	$values = array ($client_key, $subscription_limit, $description,
			$developer, $id_auth);
	$sql =& $db->prepare ('UPDATE '.DB_PREFIX.'tupdate_auth SET client_key = ?, subscription_limit = ?, description = ?, developer = ? WHERE id = ?');
	$result =& $db->execute ($sql, $values);
	if (PEAR::isError ($result)) {
		echo '<strong>Error</strong>: '.$result->getMessage ().'<br />';
		return false;
...
function um_db_delete_auth ($id_auth) {
	...
	$sql =& $db->prepare ('DELETE FROM '.DB_PREFIX.'tupdate_auth WHERE id = ?');
...
function um_db_get_auth ($id_auth) {
	...
	$sql =& $db->prepare ('SELECT * FROM '.DB_PREFIX.'tupdate_auth WHERE id = ? LIMIT 1');
	$result =& $db->execute ($sql, $id_auth);
	if (PEAR::isError ($result)) {
		echo '<strong>Error</strong>: '.$result->getMessage ().'<br />';
		return NULL;
...
function um_db_get_all_auths () {
	...
	$result =& $db->query ('SELECT * FROM '.DB_PREFIX.'tupdate_auth');
...
function um_db_check_auth ($client_key, $subscription_limit) {
	...
	$sql =& $db->prepare ('SELECT * FROM '.DB_PREFIX.'tupdate_auth WHERE client_key = ? LIMIT 1');
...
function um_db_is_auth_developer ($id_auth) {
	...
	$developer =& $db->getOne ('SELECT developer FROM '.DB_PREFIX.'tupdate_auth WHERE id = ? LIMIT 1', $id_auth);
