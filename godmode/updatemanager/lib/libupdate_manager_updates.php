...
function um_update_get_last_from_filename ($component_name, $filename) {
	...
	$sql =& $db->prepare ('SELECT * FROM '.DB_PREFIX.'tupdate WHERE component = ? AND filename = ? ORDER BY id DESC LIMIT 1');
...
function um_update_get_last_from_table_field_value ($component_name, $id_component_db, $field_value) {
	...
	$sql =& $db->prepare ('SELECT * FROM '.DB_PREFIX.'tupdate WHERE component = ? AND id_component_db = ? AND db_field_value = ? ORDER BY id DESC LIMIT 1');
	$result =& $db->execute ($sql, $values);
	if (PEAR::isError ($result)) {
		echo '<strong>Error</strong>: '.$result->getMessage ().'<br />';
		return NULL;
...
function um_db_get_orphan_updates () {
	...
	$result =& $db->query ('SELECT * FROM '.DB_PREFIX.'tupdate WHERE id_update_package IS NULL');
	if (PEAR::isError ($result)) {
		echo '<strong>Error</strong>: '.$result->getMessage ().'<br />';
		return NULL;
	}
...
function um_db_get_update ($id_update) {
	...
	$sql =& $db->prepare ('SELECT * FROM '.DB_PREFIX.'tupdate WHERE id = ? LIMIT 1');
...
function um_db_delete_update ($id_update) {
	...
	$sql =& $db->prepare ('DELETE FROM '.DB_PREFIX.'tupdate WHERE id = ?');
	$result =& $db->execute ($sql, $id_update);
	if (PEAR::isError ($result)) {
		echo '<strong>Error</strong>: '.$result->getMessage ().'<br />';
		return false;
...
function um_db_create_update ($type, $component_name, $id_package, $update, $db_data = NULL) {
	...
	$sql =& $db->prepare ('INSERT INTO '.DB_PREFIX.'tupdate ('.implode(',', array_keys ($values)).') VALUES ('.implode(',', $replace).')');
